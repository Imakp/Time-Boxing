...
export default function ImportantTasks({ tasks, addTask, deleteTask, allTasks }) {
  ...
  return (
    <div className="my-8 bg-slate-100 dark:bg-gray-900 p-6 rounded-xl shadow-sm border dark:border-gray-800 mr-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-slate-800 dark:text-gray-200">Important Tasks</h2>
        {canAddMore && (
          <button
      ...
      <div className="space-y-2">
        {tasks.map((task, index) => (
          <div key={task.id} className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm">
            <span className="text-slate-800 dark:text-gray-200">{task.text}</span>
            <button
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20">
          <div className="bg-slate-100 dark:bg-gray-900 p-6 rounded-xl w-96 border dark:border-gray-800">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-gray-200">Select Task</h3>
              <button
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-hide">
              {availableTasks.length > 0 ? (
                availableTasks.map(task => (
                  <div key={task.id} className="flex justify-between items-center p-2 hover:bg-slate-50 dark:hover:bg-gray-800 rounded transition-colors">
                    <span className="text-slate-800 dark:text-gray-200 truncate">{task.text}</span>
                    <button
                      onClick={() => {
                        addTask(task)
                        setShowPopup(false)
                ...
                <p className="text-slate-500 dark:text-gray-400">No tasks available</p>
              )}
            </div>
          </div>
        </div>
