...
export default function DayChart({ tasks, allTasks, addTask, deleteTask, updateTask }) {
  ...
  return (
    <div className="my-8 bg-slate-100 dark:bg-gray-900 p-6 rounded-xl shadow-sm border dark:border-gray-800 mr-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-slate-800 dark:text-gray-200">Day Chart</h2>
        <button
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0">
            {tasks.map((task, index) => {
              const startMinutes = task.startTime.split(':')[0] * 60 + Number(task.startTime.split(':')[1])
              const endMinutes = task.endTime.split(':')[0] * 60 + Number(task.endTime.split(':')[1])
              
              // For left column (0-11:59)
              if (startMinutes < 720) { // 720 minutes = 12 hours
                ...
                return (
                  ...
                  >
                    <span className="text-xs text-blue-600 dark:text-blue-300">{task.text}</span>
                  </div>
                )
              }
            })}
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0">
            {tasks.map((task, index) => {
              ...
              if (endMinutes > 720) { // 720 minutes = 12 hours
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20">
          <div className="bg-slate-100 dark:bg-gray-900 p-6 rounded-xl w-96 border dark:border-gray-800">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-gray-200">Add Time Block</h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={e => setNewEntry({...newEntry, task: JSON.parse(e.target.value)})}
                >
                  <option value="">Select Task</option>
                  {allTasks.filter(task => !tasks.some(t => t.id === task.id)).map(task => (
      ...
      {showEditPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20">
          <div className="bg-slate-100 dark:bg-gray-900 p-6 rounded-xl w-96 border dark:border-gray-800">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-gray-200">Edit Time Block</h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={e => setSelectedTask({...selectedTask, task: JSON.parse(e.target.value)})}
                >
                  {allTasks.filter(task => task.id === selectedTask.id || !tasks.some(t => t.id === task.id)).map(task => (
                    <option key={task.id} value={JSON.stringify(task)}>
                      {task.text}
                    </option>
                  ))}
